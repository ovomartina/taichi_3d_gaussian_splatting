...
class Lidar:
    ...
    def __init__(
        ...
        transform_world_camera: Union[np.ndarray, torch.Tensor],
    ):
        self.point_cloud = point_cloud
        self.transform_camera_lidar = transform_camera_lidar
        self.transform_world_camera = transform_world_camera
    ...
    def lidar_points_to_camera(
        ...
        if projective_transform.dtype != lidar_point_cloud.dtype:
            ...
            projective_transform = projective_transform.to(
                lidar_point_cloud.dtype)
        ...
        depth_map = torch.full(
            (image_size[1], image_size[0]), -1.0, dtype=lidar_point_cloud.dtype, device="cuda")
        depth_map[v[:], u[:]] = lidar_point_cloud[:, 2]
        # depth_map = torch.flip(depth_map, [1])

        return depth_map

    def lidar_points_to_colmap(
        self,
        lidar_point_cloud,
        ...
        lidar_point_cloud = (1/scale) * lidar_point_cloud
        
        lidar_point_cloud_homogeneous = torch.hstack((
            lidar_point_cloud, torch.ones(
                (lidar_point_cloud.shape[0], 1)).cuda()
        ))
              
        if T_colmap_world.dtype != lidar_point_cloud_homogeneous.dtype:
            # Convert T_world_colmap to the data type of lidar_point_cloud_homogeneous
            T_colmap_world = T_colmap_world.to(
                lidar_point_cloud_homogeneous.dtype)
        transformed_points = torch.matmul(T_colmap_world.cuda(
        ), torch.transpose(lidar_point_cloud_homogeneous, 0, 1))
    ...
    def lidar_points_visible(self,
        ...
        T_camera_world = inverse_SE3(self.transform_world_camera) #inverse_SE3(T_world_camera)
        if T_camera_world.dtype != lidar_point_cloud_homogeneous.dtype:
            T_camera_world = T_camera_world.to(
                lidar_point_cloud_homogeneous.dtype)
        transformed_points = torch.matmul(T_camera_world.cuda(
        ), torch.transpose(lidar_point_cloud_homogeneous, 0, 1))
        ...
        if projective_transform.dtype != transformed_points.dtype:
            projective_transform = projective_transform.to(
                transformed_points.dtype)
            
        normalized_point = torch.matmul(
            projective_transform, transformed_points) / transformed_points[2]
        ...
        self.point_cloud = visible_points
