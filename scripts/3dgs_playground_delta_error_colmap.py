...
T_world_colmap = torch.tensor([[-0.07453163,  0.11889557 ,-0.4673894,  -1.59774687],
    ...
    def start(self):
        ...
        with open(self.config.json_file_path) as f:
            ...
            for index, view in enumerate(d):
                ...
                # DEBUG ==================================                
                df = pd.read_json(
                     "/media/scratch1/mroncoroni/git/taichi_3d_gaussian_splatting/data/replica_colmap/room_1_high_quality_500_frames/val.json", orient="records")
                colmap_T_colmap_camera = torch.tensor(
                    df.iloc[index]["T_pointcloud_camera"])
                print(f"colmap_T_colmap_camera: \n\t{colmap_T_colmap_camera}")
                scale = np.mean([np.linalg.norm(T_world_colmap[:3, 0]), np.linalg.norm(T_world_colmap[:3, 1]),
                            np.linalg.norm(T_world_colmap[:3, 2])])
        
                print(f"scale: {scale}")
                groundtruth_T_colmap_camera = torch.matmul(T_world_colmap, colmap_T_colmap_camera)
                groundtruth_T_colmap_camera[:3, :3] = (1/scale)*groundtruth_T_colmap_camera[:3, :3]
                print(f"groundtruth_T_colmap_camera: \n\t{groundtruth_T_colmap_camera}")
                initial_q, initial_t = SE3_to_quaternion_and_translation_torch(groundtruth_T_colmap_camera.unsqueeze(0))
                initial_q_numpy = initial_q.detach().cpu().numpy()
                initial_t_numpy = initial_t.detach().cpu().numpy()
                initial_rotation = sym.Rot3(
                    initial_q_numpy[0, :])
                initial_pose = sym.Pose3(
                    R=initial_rotation, t=initial_t_numpy.T.astype("float"))
