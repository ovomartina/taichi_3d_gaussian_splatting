...
import argparse
from taichi_3d_gaussian_splatting.GaussianPointTrainerBundleAdjustment import GaussianPointTrainerBundleAdjustment
import os
import shutil
import numpy as np
import yaml
...
if __name__ == "__main__":
    ...
    if args.gen_template_only:
        config = GaussianPointTrainerBundleAdjustment.TrainConfig()
        # convert config to yaml
        config.to_yaml_file(args.train_config)
        exit(0)
    config = GaussianPointTrainerBundleAdjustment.TrainConfig.from_yaml_file(
        args.train_config)
    ...
    trainer = GaussianPointTrainerBundleAdjustment(config)
    trainer.train()
