...
def add_lidar(json_path:str, lidar_folder_path:str, lidar_frequency:int, noisy_lidar_measurements:bool):
    ...
    for entry in data:
        ...
        if not noisy_lidar_measurements:
            try:
                lidar_file = [s for s in lidar_files if f"_{(frame_number//lidar_frequency)}." in s][0]
            except:
                lidar_file = None
        else:
            if frame_number%lidar_frequency == 0:
                try:
                    lidar_file = [s for s in noisy_lidar_files if f"_{(frame_number//lidar_frequency)}_" in s][0]
                except:
                    lidar_file = None
            else:
                lidar_file = None
