...
import argparse
from taichi_3d_gaussian_splatting.GaussianPointTrainerContinuousBundleAdjustment import GaussianPointTrainerContinuousBundleAdjustment
import os
import shutil
import numpy as np
import yaml
...
if __name__ == "__main__":
    ...
    if args.gen_template_only:
        config = GaussianPointTrainerContinuousBundleAdjustment.TrainConfig()
        # convert config to yaml
        config.to_yaml_file(args.train_config)
        exit(0)
    config = GaussianPointTrainerContinuousBundleAdjustment.TrainConfig.from_yaml_file(
        args.train_config)
    ...
    trainer = GaussianPointTrainerContinuousBundleAdjustment(config)
    trainer.train()
