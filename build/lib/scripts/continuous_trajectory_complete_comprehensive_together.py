...
class PoseEstimator():
    ...
    def start(self):
        self.output_path = self.output_path+"_old"
        ...
        with open(self.config.json_file_path) as f:
            ...
            optimizer_bspline_bases_t = torch.optim.Adam(
                [pypose_bspline_knots_t], lr=1e-4, betas=(0.9, 0.999))  # n_basesx3
            ...
            for epoch in range(num_epochs):
                ...
                for i, current_pose in enumerate(current_pose_tensor):
                    predicted_image, predicted_depth, _, _ = self.rasteriser(
                        GaussianPointCloudContinuousPoseRasterisation.GaussianPointCloudContinuousPoseRasterisationInput(
                            ...
                            current_pose=current_pose.float().unsqueeze(dim=0),
                            color_max_sh_band=3,
                        )
                    )
                ...
                if epoch % 100 == 0:
                    ...
                    self.f2.savefig(os.path.join(
                        self.output_path, f'full_path_2d__epoch_{epoch}.png'))
