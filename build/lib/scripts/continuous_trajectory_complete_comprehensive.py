...
class PoseEstimator():
    ...
    def extract_data(self, d):
        ...
        for view_dict in d:
            ...
            ground_truth_image_downsampled_list.append(ground_truth_image_downsampled)
            resized_depth_image_downsampled= resized_depth_image_downsampled.cuda()
            resized_depth_image_downsampled_list.append(resized_depth_image_downsampled)
    ...
    def start(self):
        self.output_path = self.output_path+"less_noise"
        ...
        with open(self.config.json_file_path) as f:
            ...
            bases = groundtruth_bases + \
                np.random.normal(loc=0, scale=0.02, size=(
                    groundtruth_bases.shape[0], groundtruth_bases.shape[1])) 
            ...
            for epoch in range(num_epochs):
                ...
                if epoch % 100 == 0:
                    ...
                    print(f"Current pose tensor grad: {current_pose_tensor.grad}") 
