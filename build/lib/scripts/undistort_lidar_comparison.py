...
def main():
    optimization_folder = "/media/scratch1/mroncoroni/git/taichi_3d_gaussian_splatting/scripts/3dgs_playground_output_50_frames_discrete"
    ...
    lidar_batch_size = 10 #5  # Number of scans to cover 360°
    delta_time_lidar = 1/lidar_frequency  # [s] between scans

    gyroscope_noise_density = 2.3e-4 #1.6968e-04  # [ rad / s / sqrt(Hz) ] #0.007
    ...
    for g, gyro in enumerate(gyro_noise):
        for a, acc in enumerate(accelerometer_noise):           
            print("current values:")
            print(gyro)
            print(acc)
            gyroscope_noise_density_discrete = gyro / \
                np.sqrt(delta_time_imu)  # [ rad / s]
            accelerometer_noise_density_discrete = acc / \
            ...
            distances = np.linalg.norm(batch_pointcloud - groundtruth_batch_pointcloud, axis=1)
            # print(mean_chamfer)
            # print(np.mean(distances))
            errors[g,a] = np.mean(distances)
    ...
    for i in range(num_batches):
        ...
        for j in range(lidar_batch_size):
            offset = 2 * lidar_frequency
            lidar_index = i*lidar_batch_size+j+offset

            if lidar_index > 400:
                continue
            
            # Select the current active bases
            active_segment =(lidar_index-offset)//lidar_frequency #20
            print("active_segment: ", active_segment)
            active_bases = bases[active_segment:active_segment+4]
            current_timestamp = (lidar_index-offset -(active_segment*lidar_frequency))/lidar_frequency # 20
            ...
            if (lidar_index+1)%lidar_batch_size==0:
                ...
                groundtruth_batch_pointcloud_pcd_l5 = groundtruth_batch_pointcloud_pcd
                groundtruth_batch_pointcloud_pcd = copy.deepcopy(groundtruth_batch_pointcloud_pcd).transform(T_l5_w)
                ...
                dist_pc1_pc2 = groundtruth_batch_pointcloud_pcd_l5.compute_point_cloud_distance(batch_pointcloud_pcd)
                dist_pc1_pc2 = np.asarray(dist_pc1_pc2)
                dist_pc2_pc1 = batch_pointcloud_pcd.compute_point_cloud_distance(groundtruth_batch_pointcloud_pcd_l5)
                dist_pc2_pc1 = np.asarray(dist_pc2_pc1)
                chamfer = np.sum(dist_pc1_pc2)/len(groundtruth_batch_pointcloud_pcd_l5.points)+ np.sum(dist_pc2_pc1)/len(batch_pointcloud_pcd.points)
    ...
    surface = ax.plot_surface(gyro_noise, accelerometer_noise, errors, cmap='viridis', alpha=0.7)
    
    reference_plane = ax.plot_surface(gyro_noise, accelerometer_noise, np.mean(distances)*np.ones_like((gyro_noise, accelerometer_noise)), color='r', alpha=0.5)
    ax.set_xlabel('Gaussian Gyroscope Noise')
    ax.set_ylabel('Gaussian Accelerometer Noise')
    # ax.set_zlabel('Transformation Error')
    ax.set_title('Transformation Error')
    fig.colorbar(surface, ax=ax, shrink=0.2, aspect=5)

    # Save the figure with high DPI
    fig.savefig('transformation_error_plot.png', dpi=600)
    import pickle

    # Save the figure
    with open('transformation_error_plot.pkl', 'wb') as f:
        pickle.dump(fig, f)
