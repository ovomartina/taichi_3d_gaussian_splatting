...
class PoseEstimator():
    ...
    def start(self):
        ...
        with open(self.config.json_file_path) as f:
            ...
            pose_estimate_current_pic = [[] for _ in range(len(d))]
            ...
            # extract all noisy poses
            f2 = pl.figure(2)
            a2d = f2.add_subplot(111)

            delta_numpy_array_q = np.random.normal(0, 0.05, (3, len(d)))
            delta_numpy_array_t = np.random.normal(0, 0.05, (3, len(d)))
            ...
            for count, view in enumerate(d):
                ...
                groundtruth_q_pointcloud_camera_numpy = groundtruth_q_pointcloud_camera.cpu(
                ).numpy().reshape((1, 4))
                groundtruth_t_pointcloud_camera_numpy = groundtruth_t_pointcloud_camera.cpu(
                ).numpy().reshape((1, 3))

                storage = np.hstack((groundtruth_q_pointcloud_camera_numpy,
                                    groundtruth_t_pointcloud_camera_numpy)).reshape((7, 1))
                gt_pose = sym.Pose3.from_storage(storage)
                delta_numpy_array = np.hstack(
                    (delta_numpy_array_q[:, count], delta_numpy_array_t[:, count]))
                initial_pose = gt_pose.retract(delta_numpy_array)
            ...
            f2.savefig(os.path.join(
                self.output_path, f'Initial_trajectory.png'), dpi=300)

            for count, view in enumerate(d):
                print(
                    f"=================Image {count}========================")
                # if count <26 or count >45:
                #     continue
                ...
                for epoch in range(num_epochs):
                    ...
                    pose_estimate_current_pic[count].append(delta_tensor)
                ...
                np.savetxt(os.path.join(
                    self.output_path, f'epochs_delta_{count}/delta_estimate_{epoch}_t.txt'), pose_estimate_current_pic)

                delta_tensor = torch.cat(
                    (delta_tensor_q, delta_tensor_t), axis=0)
                current_pose = initial_pose.retract(
                    delta_tensor.clone().detach().cpu().numpy(), epsilon=epsilon)
                optimized_poses_lie[count,
                                    :] = current_pose.to_tangent().reshape(1, 6)
            ...
            np.save(os.path.join(self.output_path,
                    "optimized_poses_lie.out"), optimized_poses_lie)
            np.save(os.path.join(self.output_path,
                    "groundtruth_poses_lie.out"), groundtruth_poses_lie)
